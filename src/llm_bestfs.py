...
This file implements a best-first search approach to solving puzzles. Each node 
receives a value from the LLM, and we maintain a max heap to select the best node.
The key difference from lfs is that after each action, we compare the value
of the current node with the top of the heap and switch if the heap node is better.
...
def run_countdown(args, model):
    """Run the Countdown puzzle solver using Best-First Search approach"""
    ...
    output_path = get_game_output_path(args, "bestfs", "countdown")
    ...
    for game_idx, game in enumerate(tqdm(batch, desc=f"Batch {args.batch_num}")):
        ...
        for iter_idx in tqdm(range(completed_iterations, args.num_its), desc=f"Game {game_idx}"):
            ...
            # Track backtracking - in BestFS we backtrack when switching nodes 
            backtrack_count = 0
            ...
            while True:
                # Check stopping condition based on token usage
                ...
                # In BestFS, we always check if there's a better node to explore
                # Evaluate all nodes with unexpanded children
                best_node = current_node
                best_value = current_node.value if current_node.value is not None else float("-inf")
                
                for node in explorer.nodes_with_unexpanded_children:
                    ...
                    # Keep track of node with highest value
                    if node_value > best_value:
                        best_value = node_value
                        best_node = node
                
                # Check if we found a better node
                if len(explorer.nodes_with_unexpanded_children) == 0:
                    print("No more nodes with unexpanded children to explore!")
                    break
                
                # If best node is different from current node, switch to it
                if best_node != current_node:
                    print(f"Switching to better node {best_node} (value: {best_value} > {current_node.value})")
                    current_node = best_node
                    backtrack_count += 1  # Increment backtrack count when we switch nodes
                else:
                    print(f"Continuing with current node {current_node} (it has the best value)")
                
                # Check if node is not already expanded
                ...
                if not current_node.game_node.is_game_over():
                    ...
                    # Only consider unexpanded children as possible next nodes
                    possible_next_nodes = [child for child in current_node.children if not child.expanded]
                    if len(possible_next_nodes) == 0: raise ValueError("No more nodes with unexpanded children to explore, this shouldn't happen.")
                ...
                atts += 1  # Increment the attempt counter for each iteration
                
                # Update metrics
                metrics_log_entry = update_metrics_log(metrics_log, atts, explorer, token_usage, backtrack_count)
                if metrics_log_entry:
                    metrics_log_entry["nodes_with_unexpanded_children"] = len(explorer.nodes_with_unexpanded_children)
                    metrics_log_entry["estimated_value"] = current_node.value if current_node is not None else 0
                ...
                if current_node.game_node.is_game_over():
                    if current_node.game_node.game_result() > 0:
                        ...
                        print("That didn't work, we'll select a new path on the next iteration")
                        current_node.value = float("-inf")

                # Check if there are any nodes left to explore
                if not explorer.nodes_with_unexpanded_children:
                    print("No more nodes with unexpanded children to explore!")
                    break
            ...
            outputs.append({
                ...
                "explorer": explorer,
                "won": current_node.game_node.game_result() > 0 if current_node is not None else False,
                "attempts": atts,
                "token_usage": token_usage,
                "time_taken": end - start,
                "metrics_log": metrics_log,
                "explore_enabled": False,  # No explicit explore decisions in BestFS
...
def run_sudoku(args, model):
    """Run the Sudoku puzzle solver using Best-First Search approach"""
    
    # Create the agent - directly use SudokuAgent
    agent = SudokuAgent(
        model=model,
        model_name=args.model_name,
        model_type=args.model_type,
    ...
    # Get output path using common utility
    output_path = get_game_output_path(args, "bestfs", "sudoku")
    ...
    for puzzle_idx, puzzle in enumerate(tqdm(batch, desc=f"Batch {args.batch_num}")):
        ...
        for iter_idx in tqdm(range(completed_iterations, args.num_its), desc=f"Puzzle {puzzle_idx}"):
            ...
            try:
                # Main solving loop with Best-First Search approach
                while True:
                    # Check stopping condition based on token usage
                    total_tokens, limit_exceeded = calculate_and_check_token_usage(token_usage, args.max_token_usage)
                    if limit_exceeded:
                    ...
                    if not current_node.expanded:
                        ...
                        # Evaluate all possible moves from this node's children
                    ...
                    if not current_node.game_node.is_game_over():
                        ...
                        if len(possible_next_nodes) == 0:
                            print("No more unexpanded children to explore!")
                            break
                        ...
                        for child in possible_next_nodes:
                            # If a child doesn't have a value (shouldn't happen), use 0
                            child_value = child.value if child.value is not None else 0
                            if child_value > best_value:
                                best_value = child_value
                        ...
                        if best_child is None:
                            print("No child found with a value. This shouldn't happen.")
                            break
                        ...
                        # For tracking move accuracy - check if this is a correct move
                        ...
                        # Check if the selected node is expanded and evaluate its children if needed
                        if not selected_node.expanded:
                            selected_node.expand()
                            # Evaluate all possible moves from this node's children
                    ...
                    metrics_log_entry = update_sudoku_metrics_log(
                        metrics_log=metrics_log,
                        step=atts,
                        explorer=explorer,
                    ...
                    metrics_log_entry["estimated_value"] = best_value
            ...
            final_board = current_node.game_node.board if current_node is not None else board
            ...
            outputs.append({
                ...
                "time_taken": end - start,
                "metrics_log": metrics_log,
                "explore_enabled": False,  # No explicit explore decisions, using best-first approach
